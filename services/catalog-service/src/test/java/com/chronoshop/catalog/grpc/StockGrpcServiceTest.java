...
class StockGrpcServiceTest {
  ...
  @Mock private StreamObserver<StockCheckResponse> checkObserver;
  @Mock private StreamObserver<StockReserveResponse> reserveObserver;

  @InjectMocks private StockGrpcService service;
  ...
  void checkStock_returnsFoundTrue_whenWatchExists() {
    ...
    service.checkStock(
        StockCheckRequest.newBuilder().setWatchId(10L).setRequestedQuantity(2).build(),
        checkObserver);
  ...
  void checkStock_setsAvailableFalse_whenRequestedExceedsStock() {
    ...
    service.checkStock(
        StockCheckRequest.newBuilder().setWatchId(10L).setRequestedQuantity(5).build(),
        checkObserver);
  ...
  void checkStock_returnsFoundFalse_whenWatchMissing() {
    ...
    service.checkStock(
        StockCheckRequest.newBuilder().setWatchId(99L).setRequestedQuantity(1).build(),
        checkObserver);
  ...
  void reserveStock_returnsSuccess_whenAdjustSucceeds() {
    ...
    service.reserveStock(
        StockReserveRequest.newBuilder().setWatchId(10L).setDelta(-2).build(), reserveObserver);

    ArgumentCaptor<StockReserveResponse> captor =
        ArgumentCaptor.forClass(StockReserveResponse.class);
  ...
  void reserveStock_returnsFailure_whenInsufficientStock() {
    when(watchService.adjustStock(10L, -50))
        .thenThrow(new InsufficientStockException("Submariner", 50, 5));

    service.reserveStock(
        StockReserveRequest.newBuilder().setWatchId(10L).setDelta(-50).build(), reserveObserver);
