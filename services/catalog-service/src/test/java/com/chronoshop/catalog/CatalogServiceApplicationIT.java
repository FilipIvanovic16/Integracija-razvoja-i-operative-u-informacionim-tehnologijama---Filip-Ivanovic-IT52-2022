  ...
  void createAndSearchWatch_worksThroughRealDatabase() {
    ...
    BrandResponse brand =
        restTemplate
            .exchange(
                "/api/brands",
                HttpMethod.POST,
                new HttpEntity<>(brandReq, adminHeaders),
                BrandResponse.class)
            .getBody();
    ...
    CategoryResponse category =
        restTemplate
            .exchange(
                "/api/categories",
                HttpMethod.POST,
                new HttpEntity<>(categoryReq, adminHeaders),
    ...
    WatchRequest watchReq =
        new WatchRequest(
            ...
            null,
            null);
    var watchResponseEntity =
        restTemplate.exchange(
            "/api/watches",
            HttpMethod.POST,
            new HttpEntity<>(watchReq, adminHeaders),
            WatchResponse.class);
    assertThat(watchResponseEntity.getStatusCode()).isEqualTo(HttpStatus.CREATED);

    var searchResponse =
        restTemplate.getForObject("/api/watches?search=Master Collection", String.class);
  ...
  void createWatch_withNonAdminRole_isForbidden() {
    ...
    var response =
        restTemplate.exchange(
            "/api/watches",
            HttpMethod.POST,
            new HttpEntity<>(watchReq, customerHeaders),
            String.class);
