...
/**
 * Za razliku od StockGrpcServiceTest (poziva metode direktno, mimo mreze), ovde se StockGrpcService
 * stvarno izlaze preko gRPC in-process servera i pogadja pravim generisanim blocking stub-om -
 * proverava da protobuf serijalizacija/deserijalizacija i StockServiceGrpc kod generisan iz
 * stock.proto zaista rade zajedno, bez potrebe za mrežnim portom ili bazom.
 */
...
class StockGrpcServiceInProcessTest {
  ...
  void checkStock_realWireRoundTrip_returnsExpectedValues() {
    ...
    StockCheckResponse response =
        stub.checkStock(
            StockCheckRequest.newBuilder().setWatchId(1L).setRequestedQuantity(2).build());
