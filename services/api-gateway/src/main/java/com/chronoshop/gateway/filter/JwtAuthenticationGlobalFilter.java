...
/**
 * Jedino mesto u sistemu koje validira JWT (nasuprot dosadasnjeg gateway-header "trust" obrasca u
 * catalog/order/payment servisima). Validan token se pretvara u X-User-Id / X-User-Roles zaglavlja
 * koje ti servisi vec citaju - njihove GatewayHeader filtere ne treba menjati, samo od sada tim
 * zaglavljima zaista prethodi provera potpisa.
 */
@Component
public class JwtAuthenticationGlobalFilter implements GlobalFilter, Ordered {
  ...
  public JwtAuthenticationGlobalFilter(
      @Value("${app.jwt.secret}") String secret, PublicRouteMatcher publicRouteMatcher) {
  ...
  private Mono<Void> unauthorized(ServerWebExchange exchange, String message) {
    ...
    ApiErrorBody error =
        ApiErrorBody.of(401, "Unauthorized", message, exchange.getRequest().getURI().getPath());
    byte[] bytes;
    try {
