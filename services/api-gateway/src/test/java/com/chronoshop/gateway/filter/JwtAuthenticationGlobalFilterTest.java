...
class JwtAuthenticationGlobalFilterTest {
    ...
    private static final String SECRET = "Z2VuZXJpc2FuLXRham5pLWtsanVjLXphLWNocm9ub3Nob3AtMjAyNi1lb25pcw==";
    ...
    void protectedRoute_withInvalidToken_returns401() {
        ServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.get("/api/orders").header("Authorization", "Bearer garbage-token"));
        GatewayFilterChain chain = mock(GatewayFilterChain.class);
    ...
    void protectedRoute_withValidToken_forwardsUserHeaders() {
        ...
        ServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.get("/api/admin/orders").header("Authorization", "Bearer " + token));
        GatewayFilterChain chain = mock(GatewayFilterChain.class);
        when(chain.filter(any())).thenReturn(Mono.empty());

        StepVerifier.create(filter.filter(exchange, chain)).verifyComplete();

        verify(chain).filter(argThat(mutated ->
                "7".equals(mutated.getRequest().getHeaders().getFirst("X-User-Id"))
                        && "ADMIN".equals(mutated.getRequest().getHeaders().getFirst("X-User-Roles"))
                        && "admin@chronoshop.rs".equals(mutated.getRequest().getHeaders().getFirst("X-User-Email"))));
    }
