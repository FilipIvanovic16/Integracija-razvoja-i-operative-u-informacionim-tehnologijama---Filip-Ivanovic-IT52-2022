...
/**
 * order-service publikuje order.created i konzumira payment.completed/payment.failed
 * sa iste topic razmene "chronoshop.events". Svaki red za slusanje ima dead-letter
 * razmenu - posle iscrpljenih retry pokusaja (vidi application.properties,
 * spring.rabbitmq.listener.simple.retry.*), Spring AMQP odbija poruku bez requeue-a
 * i ona zavrsava u odgovarajucem DLQ-u umesto da se gubi ili blokira red.
 */
@Configuration
public class RabbitMqConfig {
    ...
    public Binding paymentFailedBinding(Queue paymentFailedQueue, TopicExchange chronoShopEventsExchange) {
        return BindingBuilder.bind(paymentFailedQueue).to(chronoShopEventsExchange)
