...
public class OrderService {
    ...
    for (OrderItemRequest itemReq : req.items()) {
      ...
      if (!watch.active()) {
        throw new BadRequestException(
            "Sat '" + watch.name() + "' trenutno nije dostupan za prodaju.");
      }
      if (itemReq.quantity() > watch.stockQuantity()) {
        throw new InsufficientStockException(
            watch.name(), itemReq.quantity(), watch.stockQuantity());
      }
      watches.add(watch);
    }
  ...
  public PageResponse<OrderResponse> listForUser(Long userId, Pageable pageable) {
    return PageResponse.from(
        orderRepository.findByUserId(userId, pageable), EntityMapper::toOrderResponse);
  }
  ...
  private void applyShipping(Order order, Long userId, CreateOrderRequest req) {
    if (req.addressId() != null) {
      ...
      if (req.shippingStreet() == null
          || req.shippingCity() == null
          || req.shippingPostalCode() == null
          || req.shippingCountry() == null) {
