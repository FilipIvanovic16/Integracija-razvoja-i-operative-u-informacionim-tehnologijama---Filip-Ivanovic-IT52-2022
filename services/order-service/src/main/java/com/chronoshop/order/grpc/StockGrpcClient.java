...
/**
 * Tanak omotac oko generisanog gRPC blocking stub-a. StockClient je taj koji odlucuje
 * kada preci na REST fallback - ova klasa samo propagira StatusRuntimeException dalje.
 */
@Component
public class StockGrpcClient {
    ...
    public StockCheckResponse checkStock(Long watchId, int requestedQuantity) throws StatusRuntimeException {
        return stub.withDeadlineAfter(2, TimeUnit.SECONDS)
                .checkStock(StockCheckRequest.newBuilder()
                        .setWatchId(watchId)
    ...
    public StockReserveResponse reserveStock(Long watchId, int delta) throws StatusRuntimeException {
        return stub.withDeadlineAfter(2, TimeUnit.SECONDS)
                .reserveStock(StockReserveRequest.newBuilder()
                        .setWatchId(watchId)
                        .setDelta(delta)
                        .build());
    }
