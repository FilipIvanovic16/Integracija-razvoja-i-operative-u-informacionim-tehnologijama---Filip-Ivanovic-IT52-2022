...
/**
 * CheckStock/ReserveStock preko gRPC-a, sa REST fallback-om pri nedostupnosti - tacno po
 * specifikaciji. gRPC (StockGrpcClient) je primarni put; StatusRuntimeException (server nedostupan,
 * timeout, ...) je jedini signal koji prebacuje na postojeci REST CatalogClient iz
 * feat/order-service PR-a. Poslovna pravila (sat ne postoji, nedovoljno na stanju) NISU
 * "nedostupnost" - vracaju se direktno kao izuzeci, bez REST poziva, jer bi REST vratio istu
 * poslovnu grešku.
 */
@Component
public class StockClient {
  ...
  public WatchStockInfo checkStock(Long watchId, int requestedQuantity) {
    try {
      ...
      return new WatchStockInfo(
          watchId,
          response.getWatchName(),
