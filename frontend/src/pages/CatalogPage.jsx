...
const EMPTY_FILTERS = {
  brandIds: [], categoryIds: [], genders: [], movements: [],
  conditions: [], materials: [], documentations: [],
  minPrice: '', maxPrice: '', preOwned: false,
}
...
function FilterModal({ open, onClose, brands, categories, applied, onApply }) {
  ...
  useEffect(() => {
    if (open) { setLocal(applied); setBrandSearch('') }
  }, [open])
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="filter-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="filter-list">
                  {filteredBrands.map(b => (
                    <button key={b.id} type="button"
                      ...
                      <span className="filter-option-box" />{b.name}
                    </button>
                  ))}
                </div>
              </>
              ...
              <div className="filter-list">
                {categories.map(c => (
                  <button key={c.id} type="button"
                    ...
                    <span className="filter-option-box" />{c.name}
                  </button>
                ))}
              </div>
            )}
              ...
              <div className="filter-list">
                {GENDERS.map(g => (
                  <button key={g.value} type="button"
                    ...
                    <span className="filter-option-box" />{g.label}
                  </button>
                ))}
              </div>
            )}
              ...
              <div className="filter-list">
                {MOVEMENTS.map(m => (
                  <button key={m.value} type="button"
                    ...
                    <span className="filter-option-box" />{m.label}
                  </button>
                ))}
              </div>
            )}
              ...
              <div className="filter-list">
                {CONDITIONS.map(c => (
                  <button key={c.value} type="button"
                    ...
                    <span className="filter-option-box" />{c.label}
                  </button>
                ))}
              </div>
            )}
              ...
              <div className="filter-list">
                {DOCUMENTATIONS.map(d => (
                  <button key={d.value} type="button"
                    ...
                    <span className="filter-option-box" />{d.label}
                  </button>
                ))}
              </div>
            )}
                ...
                <div className="filter-presets">
                  {PRICE_PRESETS.map(pr => (
                    <button key={pr.label} type="button"
                      ...
                      onClick={() => setLocal(p => ({ ...p, minPrice: pr.min, maxPrice: pr.max }))}>
                      {pr.label}
                    </button>
                  ))}
                </div>
        ...
        <div className="filter-modal-footer">
          <button className="btn btn-ghost" onClick={() => { setLocal(EMPTY_FILTERS); onApply(EMPTY_FILTERS); onClose() }}>
            Resetuj sve
          </button>
...
function buildChips(filters, brands, categories) {
  ...
  if (filters.minPrice || filters.maxPrice) {
    ...
    const label = filters.minPrice && filters.maxPrice
      ...
      : filters.minPrice ? `Od €${fmt(filters.minPrice)}`
...
export default function CatalogPage() {
  ...
  function applyFilters(f) { setFilters(f); setPage(0) }
  ...
  const pageTitle = urlCondition === 'NEW' && filters.conditions.includes('NEW')
    ? 'Novi satovi'
  ...
  return (
    <div>
      ...
      <div className="toolbar">
        ...
        <select className="select" value={sort} onChange={e => { setSort(e.target.value); setPage(0) }}>
        ...
        <div className="filter-chips-row">
          {chips.map(chip => (
            <span key={`${chip.key}-${chip.val}`} className="filter-chip">
              {chip.label}
              <button className="filter-chip-remove" onClick={() => removeChip(chip.key, chip.val)}>✕</button>
            </span>
          ))}
          <button className="filter-clear-all" onClick={() => { setFilters(EMPTY_FILTERS); setPage(0) }}>
            Ukloni sve
          </button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <>
          <div className="grid watch-grid">
            {data.content.map(w => <WatchCard key={w.id} watch={w} />)}
          </div>
