  ...
  if (error)
    return (
      <div className="container">
        <div className="alert alert-error mt">{error}</div>
      </div>
    )
        ...
        <div className="detail-layout">
          ...
          <div className="detail-gallery">
            ...
            <div className="gallery-thumbs">
              {thumbImages.map((src, i) => (
                ...
                >
                  {src ? <img src={src} alt={`${watch.name} ${i + 1}`} /> : <span>⌚</span>}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="detail-info">
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
              <div
                className={`availability-badge ${watch.inStock ? 'avail-in' : 'avail-out'}`}
              ...
              {watch.condition && (
                <div
                  className={`availability-badge tag-cond-${watch.condition}`}
                  style={{ marginBottom: 0 }}
                >
                  {watch.condition === 'NEW'
                    ? 'Nov'
                    : `Polovan · ${CONDITION_LABELS[watch.condition]}`}
                </div>
              )}
            </div>
            ...
            <p className="detail-ref">
              Referenca: <strong>{watch.referenceNumber}</strong>
              &nbsp;·&nbsp;{watch.category?.name}
            </p>
            ...
            {/* Description */}
            {watch.description && <p className="detail-desc">{watch.description}</p>}
            ...
            {watch.inStock ? (
              <div className="detail-actions">
                <div className="qty-control">
                  ...
                  <button
                    className="qty-btn"
                    onClick={() => setQty((q) => Math.min(watch.stockQuantity, q + 1))}
                  >
                    +
                  </button>
                </div>
          ...
          <div className="specs-columns">
            <div className="spec-group">
              ...
              <table className="spec-table">
                <tbody>
                  <tr>
                    <td className="spec-k">Brend</td>
                    <td>{watch.brand?.name}</td>
                  </tr>
                  <tr>
                    <td className="spec-k">Model</td>
                    <td>{watch.name}</td>
                  </tr>
                  <tr>
                    <td className="spec-k">Referenca</td>
                    <td>{watch.referenceNumber}</td>
                  </tr>
                  <tr>
                    <td className="spec-k">Kategorija</td>
                    <td>{watch.category?.name}</td>
                  </tr>
                  <tr>
                    <td className="spec-k">Pol</td>
                    <td>{GENDER_LABELS[watch.gender] || '-'}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="spec-group">
              ...
              <table className="spec-table">
                <tbody>
                  <tr>
                    <td className="spec-k">Tip mehanizma</td>
                    <td>{MOVEMENT_LABELS[watch.movement] || '-'}</td>
                  </tr>
                  <tr>
                    <td className="spec-k">Pogon</td>
                    <td>{watch.movement === 'QUARTZ' ? 'Baterija' : 'Mehanički'}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="spec-group">
              ...
              <table className="spec-table">
                <tbody>
                  <tr>
                    <td className="spec-k">Prečnik</td>
                    <td>{watch.caseDiameterMm ? `${watch.caseDiameterMm} mm` : '-'}</td>
                  </tr>
                  <tr>
                    <td className="spec-k">Vodootpornost</td>
                    <td>{watch.waterResistanceM ? `${watch.waterResistanceM} m` : '-'}</td>
                  </tr>
                  {watch.material && (
                    <tr>
                      <td className="spec-k">Materijal</td>
                      <td>{MATERIAL_LABELS[watch.material] || watch.material}</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <div className="spec-group">
              ...
              <table className="spec-table">
                <tbody>
                  ...
                  {watch.documentation && (
                    <tr>
                      <td className="spec-k">Dokumentacija</td>
                      <td>{DOCUMENTATION_LABELS[watch.documentation]}</td>
                    </tr>
                  )}
                  <tr>
                  ...
                  <tr>
                    <td className="spec-k">Količina</td>
                    <td>{watch.stockQuantity} kom</td>
                  </tr>
                  <tr>
                    <td className="spec-k">Cena</td>
                    <td>
